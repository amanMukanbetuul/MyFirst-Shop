...
const Lga1700 = () => {
    ...
    const {shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites,getSocketLGA1700} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getSocketLGA1700(search)
        } else  {
            getSocketLGA1700()
        }
    }, [search]);
    ...
    return (
        <section>
            ...
            <div className={styles.action}>
                <h2 className={styles.title}>Socket LGA 1700</h2>
