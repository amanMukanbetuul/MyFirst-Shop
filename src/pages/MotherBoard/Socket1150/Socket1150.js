...
const Socket1150 = () => {
    ...
    const {shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites,getSocket1150} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getSocket1150(search)
        } else  {
            getSocket1150()
        }
    }, [search]);
