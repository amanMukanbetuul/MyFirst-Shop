...
const Socket1151 = () => {
    ...
    const {getAllShoes, shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites,getSocket1151} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getSocket1151(search)
        } else  {
            getSocket1151()
        }
    }, [search]);
    ...
    return (
        <section>
            ...
            <div className={styles.action}>
                <h2 className={styles.title}>Socket 1151 V2 (8th & 9th Gen)</h2>
