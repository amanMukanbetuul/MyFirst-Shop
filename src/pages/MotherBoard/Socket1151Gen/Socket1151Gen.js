...
const Socket1151Gen = () => {
    ...
    const {shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites,getSocket1151Gen} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getSocket1151Gen(search)
        } else  {
            getSocket1151Gen()
        }
    }, [search]);
    ...
    return (
        <section>
            ...
            <div className={styles.action}>
                <h2 className={styles.title}>Socket 1151 (6th & 7th Gen)</h2>
