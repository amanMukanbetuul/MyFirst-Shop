...
const LaserPr = () => {
    ...
    const {getLaserPrinter, shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getLaserPrinter(search)
        } else  {
            getLaserPrinter()
        }
    }, [search]);
    ...
    return (
        <section>
            ...
            <div className={styles.action}>
                <h2 className={styles.title}>Лазерные принтеры</h2>
