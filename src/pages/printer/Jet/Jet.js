...
import styles from './Jet.module.css'


const Jet = () => {

    const {getJetPrinter, shoes, addShoesInCart, cart, deleteShoesInCart, postFavourites, favourites, delFavourites} = useContext(CustomContext);

    const [search, setSearch] = useState('');

    useEffect(() => {
        if (search !== '') {
            getJetPrinter(search)
        } else  {
            getJetPrinter()
        }
    }, [search]);
    ...
    return (
        <section>
            ...
            <div className={styles.action}>
                <h2 className={styles.title}>Струйные принтеры</h2>
